...
var BoardLayer = cc.Layer.extend({
    ...
    onCompleted: function onCompleted(sender, type) {
    ...
    onAngle: function onAngle(sender, type) {
        if (type != ccui.Widget.TOUCH_ENDED) return;
        if (dialogOpen) {
            ...
            if (counter == 1) {
                ...
                if (g_candis[selectedType][selectedCandi].angA == this.angA && g_candis[selectedType][selectedCandi].angB == this.angB || g_candis[selectedType][selectedCandi].angA == this.angB && g_candis[selectedType][selectedCandi].angB == this.angA) {
...
var CanLayer = cc.Layer.extend({
    ...
    refresh: function refresh() {
        ...
        for (i = 0; i < g_candis.length; i++) {
            for (var j = 0; j < g_candis[i].length; j++) {
                if (g_candis[i][j].state == con.canSelected) g_candis[i][j].state = con.canNormal;
            }
        }var avails = [];
        // get available types
        for (i = 0; i < g_candis.length; i++) {
        ...
        for (i = 0; i < g_candis[selType].length; i++) {
            if (g_candis[selType][i].state == con.canNormal) candiAvails.push(i);
        } // get one candidate
        ...
        for (i = 0; i < con.typeCount; i++) {
            if (i != selType) avails.push(i);
        }candiAvails = [];
        for (i = 0; i < con.angleCount; i++) {
            if (i != g_candis[selType][selCandi].angA && i != g_candis[selType][selCandi].angB) candiAvails.push(i);
    ...
    checkSet: function checkSet() {
        ...
        for (i = 0; i < this.candidates.length; i++) {
            if (this.candidates[i].selected) {
                if (this.candidates[i].data.type == con.typeType) typeData = this.candidates[i].data;else sels.push(this.candidates[i].data);
            }
                ...
                if (!sender.isSelected()) {
                    ...
                    if (counter == 3) {
                        switch (this.checkSet()) {
                            case -2:
                                showMessageBoxOK(this, "Set already completed", "That set is already completed.", this.deselectAll);
                                break;
                            case -1:
                                showMessageBoxOK(this, "Try again", "Those 3 cards do not form a set.", this.deselectAll);
                                break;
                            default:
                                showMessageBoxOK(this, "That's a set", "Now touch respective circle to make a set.", this.getReady);
                        }
                    }
                } else {
                    counter--;
