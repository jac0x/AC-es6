...
var angleNormalValues = ["angle1_normal.png", "angle2_normal.png", "angle3_normal.png", "angle4_normal.png", 
"angle1_normal.png", "angle2_normal.png", "angle3_normal.png", "angle4_normal.png"];
...
var connectionTypes = [
    "Linear pairs", "Alternate exterior", "Alternate interior", "Same-side interior", "Vertical"
];
var g_candis = [
    [
        {
...
var isRightCandidate = function(type, angA, angB) {
    if (type < 0 || type >= g_candis.length)
        return -1;
    for (i = 0; i < g_candis[type].length; i ++)
        if ((g_candis[type][i].angA == angA && g_candis[type][i].angB == angB) || (g_candis[type][i].angA == angB && g_candis[type][i].angB == angA)){
            if (g_candis[type][i].state == con.canCompleted)
                return -2;
            return i;
        }
    return -1;
